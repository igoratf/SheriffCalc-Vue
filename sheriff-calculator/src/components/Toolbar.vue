  ...
  <div id="toolbar" class="app-component">
    <v-toolbar color="indigo" dark>
      <v-menu offset-y>
        ...
        <v-list>
          <v-list-tile>
            <v-btn flat @click="dialog = true">New Game</v-btn>
          </v-list-tile>
          <v-list-tile>
            <v-btn flat @click="toRanking()">Ranking</v-btn>
          </v-list-tile>
        </v-list>
      </v-menu>
      <v-toolbar-title
        @click.stop="toDashBoard()"
        class="home"
      >Sheriff of Nottingham Score Calculator</v-toolbar-title>
      <v-spacer></v-spacer>
      <v-toolbar-items class="hidden-sm-and-down">
        <v-dialog v-model="dialog" persistent max-width="350">
          ...
          <v-card>
            <v-card-title class="headline" :style="{color:currentColor}">
              <strong>New Game</strong>
            </v-card-title>
            <v-card-text
              :style="{color:currentColor}"
            >Are you sure you want to start a new game? All players info will be lost</v-card-text>
            <v-card-actions>
